 ...
 */
  ...
  @POST
  @Path("/appLeave/{empId}/{response}")
  @Consumes(MediaType.APPLICATION_JSON)
  @Produces(MediaType.APPLICATION_JSON)
  public final String appLeave(@PathParam("empId") final int empId, @PathParam("response") final String response,
   final LeaveDetails l) throws ParseException {
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
    String sdate = sdf.format(l.getLevStartDate());
    String edate = sdf.format(l.getLevEndDate());
    String s = LeaveDetails.applyLeave(empId, sdate, edate, l.getLevNoOfDays(), l.getLevType(), l.getLevReason(),response);
    return s;
  }
