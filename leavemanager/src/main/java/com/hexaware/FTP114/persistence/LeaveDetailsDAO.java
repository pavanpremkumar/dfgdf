...
public interface LeaveDetailsDAO  {
  ...
  /**
   * return all the details of selected leave details.
   ...
   * @return the count.
   */
  @SqlQuery("SELECT COUNT(*) FROM LEAVE_DETAILS WHERE EMP_ID= :empId AND LEV_SSL_STATUS= 'YES'")
  int sslCount(@Bind("empId") int empId);

  /**
   * return difference in months.
   * @param joindate the joining date of the employee.
   * @param today is today's date.
   * @return the employee object.
   */
  @SqlQuery("SELECT DATEDIFF (:today, :joindate)")
  int diffInMonths(@Bind("today") String cd,
                  @Bind("joindate") String jd);
   ...
   * @param levType the type of leave
   * @param levStatus the status of leave
   * @param levSslStatus the SSL status of leave.
   * @param levReason the reason for leave
   * @param levAppliedDate the applied date of the employee
   */
  @SqlUpdate("INSERT INTO LEAVE_DETAILS "
               ...
               +
               "              :levAppliedDate)")
    void insertSsl(@Bind("empId") int empId,
                @Bind("levStartDate") String levStartDate,
                @Bind("levEndDate") String levEndDate,
                @Bind("levNoOfDays") int levNoOfDays,
                @Bind("levType") LeaveType levType,
                @Bind("levStatus") LeaveStatus levStatus,
                @Bind("levSslStatus") SslStatus levSslStatus,
                @Bind("levReason") String levReason,
                @Bind("levAppliedDate") String levAppliedDate);
